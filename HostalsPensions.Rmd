...
if (mostrar.html.pt.durada.t == TRUE) {
  ...
  if (crear.informe.word == TRUE) {
    ...
    my_doc <- my_doc %>% 
      ...
      body_add_par(value = "Evolució mensual i Totals", style = "heading 2") %>% 
      body_add_par("", style = "Normal") %>% # blank paragraph
      body_add_table(my_data8.t2, style = "table_template") %>%
      #  body_add_img(src = file.path(getwd(), imgpath, paste0(anydades, ".pt.REGIM.t.png")), width = 6, height = 3, style = "centered") %>% 
      body_add_par("", style = "Normal") %>% # blank paragraph
...
```{r Idem d'Origens dels menors en taula}
...
if (crear.informe.word == TRUE) {
  ...
  my_doc <- my_doc %>% 
    body_add_break() %>% 
    body_add_par(value = "Categories", style = "heading 1") %>% 
    body_add_par("", style = "Normal") %>% # blank paragraph
    body_add_par(value = "Resum", style = "heading 2") %>% 
    body_add_par("", style = "Normal") %>% # blank paragraph
...
### Gràfic de barres de Categories

```{r Gràfic de barres de Categories, echo=TRUE}

# Gràfic de barres ara
# ----------------------------------------
## First look at a quick and dirty version of the future ggplot2 chart (prior to customizations)
#qplot(data=my_data7h, x=Categoria, geom="bar") 

# Basic barplot with ggplot
ggplot.2017.perfils <- ggplot(data=my_data7h, aes(x=Categoria, y=n)) +
  geom_bar(stat="identity") +
  #scale_x_discrete(breaks = NULL) +
  scale_x_discrete(labels = function(x) str_wrap(x, width = 10)) +
...
my_data7i <- my_data %>% 
  count(Categoria,
        ...
        sort = FALSE) %>%
  filter(!is.na(Nacionalitat)) %>%
  filter(Categoria == "Decretada o recent majoria d'edat (Refugiat o Immigrant)")
# Afegir fila a sota amb totals
