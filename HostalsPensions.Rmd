...
if (crear.html.pt.membres.t == TRUE) {
  saveWidget(pt.membres.t, file=file.path(getwd(), htmlpath, paste0(anydades, ".pt.membres.uf2.avg.t.html")), selfcontained=TRUE) 
}
...
if (crear.html.pt.membres.t == TRUE) {
  saveWidget(pt.membres.t, file=file.path(getwd(), htmlpath, paste0(anydades, ".pt.membres.uf2.max.t.html")), selfcontained=TRUE) 
}
...
if (mostrar.html.pt.durada.t == TRUE) {
  ...
  if (crear.informe.word == TRUE) {
    ...
    my_doc <- my_doc %>% 
      body_add_break() %>% 
      body_add_par("", style = "Normal") %>% # blank paragraph
      body_add_par(value = "Durada - Evolució mensual i Totals", style = "heading 2") %>% 
      body_add_par("", style = "Normal") %>% # blank paragraph
      body_add_img(src = file.path(getwd(), imgpath, paste0(anydades, ".pt.DURADA.t.png")), width = 3, height = 3, style = "centered") %>% 
      body_add_par(" Pernoctacions de persones allotjades, en qualsevol regim d'alimentació") %>%
      body_add_par("", style = "Normal") %>% # blank paragraph
      body_add_par("La intensitat del color vermell és proporcional a la suma de casos de cada tipus", style = "Balloon Text")
